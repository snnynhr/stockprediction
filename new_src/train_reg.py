...
XDir = "../newData/trainXall"
YDir = "../newData/trainYall"
weightDir = "../newData/weightAll"
...
for stock in stockSet:
	Xpath = '%s/%s.json' %(XDir,stock)
	Ypath = '%s/%s.txt' %(YDir,stock)
	weightPath = '%s/%s.txt' %(weightDir,stock)
	print "creg -x %s -y %s --l1 %s --z %s 2> /dev/null" %(Xpath,Ypath,'1',weightPath)
